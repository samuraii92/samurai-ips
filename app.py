...
# ==========================================
# تصميم لوحة التحكم (تمت إضافة أدوات التحكم)
# ==========================================
DASHBOARD_HTML = """
<!DOCTYPE html>
<html>
...
def update_token():
    ...
    if add_days_str:
        ...
        # إذا كان التوكن جديداً ولم يتصل بعد، نغير عدد أيامه الأساسية
        if row.get("status") == "NEW":
            current_duration = row.get("duration_days", 30)
            update_fields["duration_days"] = current_duration + add_days
        # إذا كان التوكن مستخدماً، نعدل تاريخ الانتهاء الفعلي
        elif row.get("expiry"):
            ...
            # إرجاع التوكن للعمل إذا كان منتهياً وتم تمديده
...
def validate():
    ...
    if not row: return jsonify({"status": "INVALID"})
    ...
    if status == 'BURNED': return jsonify({"status": "BURNED"})
...
@app.route('/')
def home():
    return "Samurai System is ONLINE and CONNECTED."
